...
int main(){
    ...
    ofstream fdata_trajectory;
    fdata_trajectory.precision(20);
    fdata_trajectory.open("normalized_trajectories.dat");
    ...
    for(int i=1; i<16; i++){
        ...
        for (int i = 0; i < n; i++){
            ...
            t += dt;
            MAE += abs(sqrt(Y[0]*Y[0] + Y[1]*Y[1])/x_start - 1.);

            fdata_trajectory << t << " " << Y[0]/x_start << " " << Y[1]/x_start << " " << Y[2]/x_start << endl;
        }
        cout << i << " " << gamma << " " << MAE/(n) << endl;
        fdata << i << " " << MAE/(n) << endl;
        fdata_trajectory << endl << endl;
    }
}
